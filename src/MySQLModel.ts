...
class MySQLModel {
	...
	getWhere(query: any): string {
		...
		for (const k in query) {
			...
			} else if (query[k]!==null && typeof query[k] === 'object') {
	...
	find(query: any = null, order: any = null, fields: any = null, paging: any = null ): Promise<Array<any> | null> {
		const self = this
		return new Promise((resolve) => {
			const where = query && self.getWhere(query)
			const orderBy = order && self.getOrder(order)
			if (!fields) fields = '*'
			if (paging) {
				const limit = paging.limit || 20
				const offset = paging.offset || 0
				MySQLModel.raw('SELECT ' + fields + ' FROM `' + self.table + '`' + (where ? ' WHERE ' + where : '') + (orderBy ? ' ORDER BY ' + orderBy : '') + ' LIMIT ' + offset + ',' + limit, [], (err, res) => {
					if (res) {
						resolve(res)
					} else {
						console.error(err)
				...
				MySQLModel.raw('SELECT ' + fields + ' FROM `' + self.table + '`' + (where ? ' WHERE ' + where : '') + (orderBy ? ' ORDER BY ' + orderBy : ''), [], (err, res) => {
					if (res) {
						resolve(res)
					} else {
